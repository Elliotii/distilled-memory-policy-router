#!/usr/bin/env python3
"""Summarize hard READ v2 downstream internal rubric review rows."""
...
def validate_review(rows: list[dict[str, Any]]) -> None:
    if len(rows) != 30:
        raise ValueError(f"expected 30 review rows, found {len(rows)}")
    seen: set[str] = set()
    for row in rows:
        ...
        if row.get("manual_utility") != expected_utility:
            raise ValueError(
                f"{prompt_id}: manual_utility={row.get('manual_utility')} "
