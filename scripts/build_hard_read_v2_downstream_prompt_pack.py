#!/usr/bin/env python3
"""Build the hard READ v2 downstream prompt-pack scaffold.

This is a deterministic file builder. It does not call APIs, run model
inference, load adapters, or score answer quality.
"""
...
SENSITIVE_PLACEHOLDER_RE = re.compile(
    r"\b[A-Z]{2,}-\d{2,}\b|reset-password-\d+\b|temporary admin password",
    re.IGNORECASE,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build hard READ v2 downstream prompt pack")
